...
if (mode == 0)
{
    ...
    if (int.TryParse(Console.ReadLine(), out int targetMode) && targetMode is >= 1 and <= 2)
    {
        ...
        var result = Generator.FindNextBonusMode(ticks, (PrintMode)targetMode);
...
{
    ulong ticks = PromptTime();
    var result = mode == 1 ? Generator.FindNextItemBonus(ticks, itemId) : Generator.FindNextBall(ticks, itemId);
